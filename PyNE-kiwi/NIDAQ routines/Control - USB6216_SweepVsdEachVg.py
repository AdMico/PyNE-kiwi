...
#Vg array
start_g = 2.0; end_g = 3.0; step_g = 0.2 #Create a up and downsweep array
count_g = int((end_g-start_g)/step_g) + 1 
Vg = np.linspace(start_g, end_g, count_g, endpoint=True) #I like linspace a bit more than arange
...
for i in range(count_g):
    print("sweep",i,"of",count_g,"with gate voltage",Vg[i])
